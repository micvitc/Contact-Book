...
def add_user():
    ...
    if _id and _name and _email and _phone and _address and request.method=='POST':
        id = mongo.db.User.insert_one({'user_id':_id,'user_name':_name,'user_email':_email,'PhoneNo':_phone,'user_address':_address})
...
def add_login():
    ...
    if _lid and _lusername and _lpwd and request.method=='POST':
        ...
        id = mongo.db.Login.insert_one({'login_id':_lid,'login_username':_lusername,'login_password':_lpwd})
...
def add_roles():
    ...
    if _rid and _rname and _rdesc and request.method=='POST':
        id = mongo.db.Roles.insert_one({'role_id':_rid,'role_name':_rname,'role_description':_rdesc})
...
@app.route('/add/ContactDetails',methods=['POST'])
def add_Contact_details():
    ...
    if _cdid and _cdphone and _cdname and _cdemail and request.method=='POST':
        id = mongo.db.Contact_Details.insert_one({'cd_id':_cdid,'cd_Phone':_cdphone,'cd_name':_cdname,'cd_email':_cdemail})
...
def add_address():
    ...
    if _lmark and _city and _place and _houseno and request.method=='POST':
        id =mongo.db.Address.insert_one({'landmark':_lmark,'city':_city,'place':_place,'houseno':_houseno})
...
def ContactDetails(id):
    Details=mongo.db.Contact_Details.find_one({'_id':ObjectId(id)})
    resp=dumps(Details)
    return resp

@app.route('/Address/<id>')
def Address(id):
    address=mongo.db.Address.find_one({'_id':ObjectId(id)})
    resp=dumps(address)
    return resp
